...
if __name__ == "__main__":
    ...
    sql = "INSERT INTO vehicle (vehicledata_id,vehID,Delay,RedArrival,SplitFailure,SignalID,ApproachDirection,TravelDirection,ETT,TravelTime,ExitStatus,Day,EntryTime,ExitTime,Stops,Uturn) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    for row in csv_data:
        row = nullify(row)
        #print(row)
        cursor.execute(sql, row)

    # sql2 = "INSERT INTO signals (signal_id, region_id, signal_number) VALUES (%s, %s, %s)"
    # cursor.execute(sql2, [None, 5, 1037])
    # cursor.execute(sql2, [None, 5, 1113])
    # cursor.execute(sql2, [None, 5, 3084])

    # sql3 = "INSERT INTO coverage (coverage_id, coverage_name) VALUES (%s, %s)"
